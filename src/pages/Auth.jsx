...
export default function AuthPage() {
  ...
  const handleEmailAuth = async (e) => {
    ...
    try {
      ...
      if (msg.toLowerCase().includes("email not confirmed")) {
        toast.error("يرجى تأكيد البريد أولاً من الرسالة المرسلة إليك");
  ...
  return (
    <div className="min-h-screen font-cairo flex flex-col items-center justify-center px-5" style={{ background: "#020203" }} dir="rtl">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="text-center space-y-1">
          ...
          <h1 className="text-2xl font-black gradient-text-white">قلمي</h1>
          <p className="text-xs text-white/40">
            {mode === "signin" ? "سجّل الدخول للوصول إلى مكتبتك" : "أنشئ حساباً جديداً"}
          </p>
        </div>
        ...
        <form onSubmit={handleEmailAuth} className="space-y-3">
          {mode === "signup" && (
            <input type="text" placeholder="الاسم (اختياري)" value={fullName} onChange={(e) => setFullName(e.target.value)}
          ...
          <input type="email" placeholder="البريد الإلكتروني" value={email} onChange={(e) => setEmail(e.target.value)} dir="ltr"
            className="w-full px-4 py-3 rounded-xl text-sm text-white placeholder-white/30 outline-none text-right"
            style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }} />
